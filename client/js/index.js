...
import { fasttalk } from "./lib/fasttalk.js";
...
window.onload = () => {
    ...
    switch (window.location.hostname) {
		...
		case "localhost": serverName = "local-network"; break;
    }
...
const socketInit = (() => {
    ...
    return port => {
        let socket = new WebSocket((window.location.protocol === "http:" ? 'ws://' : "wss://") + window.location.hostname + ':' + port);
        // Set up our socket
        ...
        socket.talk = (...message) => {
            ...
            socket.send(fasttalk.encode(message));
        };                
        ...
        socket.onmessage = function socketMessage(message) {
            ...
            let m = fasttalk.decode(message.data);
