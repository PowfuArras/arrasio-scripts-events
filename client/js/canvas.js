...
class Canvas {
    ...
    keyboardDown(event) {
		...
		case global.KEY_UPGRADE_LOCK: this.upgradeLock = true; break;
        }
        if (!event.repeat) {
            switch (event.keyCode) {
            ...
            if (global.canSkill) {
                for (let i = 0; i < (this.upgradeLock ? 12 : 1); i++) {
					switch (event.keyCode) {
    ...
    keyboardUp(event) {
		...
		case global.KEY_UPGRADE_LOCK: this.upgradeLock = false; break;
        }
    }         
    mouseDown(mouse) {
        switch (mouse.button) {
